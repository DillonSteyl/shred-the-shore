...
func _ready() -> void:
	...
	var decay_rate_tween = get_tree().create_tween()
	decay_rate_tween.tween_property(
		self, "decay_per_second", END_DECAY_PER_SECOND, Constants.DIFFICULTY_INCREASE_DURATION
	)
